...
demo <- strandedCounts(bamsplus, ext=50, spacing=20, filter=5, param=param, BPPARAM=MulticoreParam(), width = win_width, bin=F)

#local background
surrounds <- 750
neighbor <- suppressWarnings(resize(rowRanges(demo), surrounds, fix="center"))
wider <- regionCounts(bamsplus, regions=neighbor, ext=50, param=param4neighbour)
...
binned <- strandedCounts(bamsplus, bin=TRUE, width=10000, param=param, BPPARAM=MulticoreParam())
...
write.table(df_sigwin, file="sig_win_plus.bed", quote=F, sep="\t", row.names=F, col.names=F)
write.table(df_fdrsig, file="merged_win_plus.bed", quote=F, sep="\t", row.names=F, col.names=F)
